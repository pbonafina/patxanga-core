-- ============================================================
-- PATXANGA - START MATCH FROM LOBBY TEST
-- Version: 1.0
-- ============================================================
...
begin
    ...
    perform public.accept_patxanga_invite(
        p_invite_id := v_invite_id,
        p_user_id := v_guest_user
    );

    v_start_result := public.start_patxanga_match_from_lobby(
        p_match_id := v_match_id,
        p_host_player_id := v_host_player_id
    );

    raise notice 'Start from lobby result: %', v_start_result;

    raise notice 'Lobby status after: %',
    (
        select status
    ...
    raise notice 'Match status after: %',
    (
        select status
