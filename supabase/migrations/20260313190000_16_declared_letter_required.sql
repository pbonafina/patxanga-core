-- ============================================================
-- PATXANGA - hydrate_patxanga_placed_tiles()
-- Version: 1.1
-- Purpose: Require declared_letter for wildcard tiles
-- ============================================================
...
begin
    ...
    loop
        ...
        if v_special_type = 'wildcard' then
            if v_declared_letter is null then
                raise exception 'declared_letter is required for wildcard tile %', v_tile_id;
            end if;

            if char_length(v_declared_letter) <> 1 then
                raise exception 'declared_letter must be exactly 1 character for wildcard tile %', v_tile_id;
            end if;
