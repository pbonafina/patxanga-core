# PATXANGA — DISTRIBUIÇÃO OFICIAL DE PEÇAS
Versão: 1.1
Status: Oficial
Duração alvo: 20–30 minutos
Ritmo: Equilibrado
Especiais: Ocasionais

---

## 1. Estrutura Geral

Total de peças na bolsa: **110**

- Letras comuns: 103
- Peças especiais: 7

---

## 2. Peças Especiais

Todas as peças especiais podem substituir qualquer letra (wildcard).

| Peça | Quantidade | Pontos Base | Efeito |
|------|------------|-------------|--------|
| Coringa | 2 | 0 | Substitui qualquer letra |
| Pular Turno | 4 | 0 | Substitui qualquer letra + faz jogador escolhido pular próximo turno |
| Patxanga Real | 1 | 0 | Substitui qualquer letra + dobra a pontuação da palavra principal |

Total especiais: **7**

Observação importante:
- A Patxanga Real dobra **apenas a palavra principal da jogada**.
- Não dobra palavras secundárias nem a soma total da jogada.

---

## 3. Letras Comuns (103)

Não existe peça "Ç".  
Quando necessário, utiliza-se a letra **C** normalmente.

---

### 3.1 Vogais (46)

| Letra | Qtde | Pontos |
|-------|------|--------|
| A | 14 | 1 |
| E | 11 | 1 |
| O | 9 | 1 |
| I | 7 | 1 |
| U | 5 | 2 |

Total vogais: **46**

---

### 3.2 Consoantes Frequentes (34)

| Letra | Qtde | Pontos |
|-------|------|--------|
| S | 7 | 1 |
| R | 6 | 1 |
| N | 5 | 1 |
| D | 4 | 2 |
| M | 4 | 2 |
| T | 4 | 2 |
| C | 4 | 2 |

Total: **34**

---

### 3.3 Consoantes Intermediárias (14)

| Letra | Qtde | Pontos |
|-------|------|--------|
| L | 3 | 2 |
| P | 2 | 3 |
| B | 2 | 3 |
| G | 2 | 3 |
| V | 2 | 3 |
| F | 1 | 4 |
| H | 1 | 4 |
| J | 1 | 5 |

Total: **14**

---

### 3.4 Letras Estratégicas (9)

| Letra | Qtde | Pontos |
|-------|------|--------|
| Q | 2 | 6 |
| X | 2 | 6 |
| Z | 2 | 7 |
| K | 1 | 7 |
| Y | 1 | 7 |
| W | 1 | 7 |

Total: **9**

---

## 4. Total Geral

46 (vogais)  
+ 34 (consoantes frequentes)  
+ 14 (intermediárias)  
+ 9 (estratégicas)  

= **103 letras**

+ 7 especiais  

= **110 peças totais**

---

## 5. Justificativa Matemática

- Vogais ~41% da bolsa
- Boa fluidez lexical
- Letras raras mantêm tensão estratégica
- Pontuação média por palavra controlada
- Patxanga Real gera picos raros e decisivos na palavra principal
- Peças de Pular Turno aparecem ocasionalmente sem dominar o ritmo

---

## 6. Ordem Oficial de Cálculo de Pontuação

1. Calcular pontuação da palavra principal
2. Calcular pontuação das palavras secundárias (cruzamentos)
3. Aplicar bônus por uso de 7 peças à jogada (se houver)
4. Aplicar multiplicador da Patxanga Real **somente sobre a palavra principal**, se usada

---

## 7. Status

Distribuição Oficial Patxanga v1.1 consolidada.
Matematicamente fechada.
Pronta para implementação técnica.